...
notebook.add(tab_risk, text='Risk')
...
<p style=font-size:85%;><a href='https://github.com/mosajd' style='color:red;'>GMS (Global Maritime Search)</a> software which is the result of one of the teamwork projects for\
...
weather_mean_label = ttk.Label(lf_risk_weather, text="Weather Mean (days): ")
...
weather_std_label = ttk.Label(lf_risk_weather, text="Weather STD (days): ")
...
lf_simulation.grid(column=0,row=3, padx=10, pady=5, ipady=5, ipadx=5, sticky=tk.EW)
...
########################################################################################## Button Simulation #####################################################################################
# Button Simulation
simulation_button = ttk.Button(tab_risk, text="Simulate", command=lambda: graphTimeCost(stochasticTimeCost()))
simulation_button.grid(column=0, row=4, pady=15, ipadx=5, ipady=5)
...
def stochasticTimeCost():
    ...
    df['total_time'] = (df['bunkering']/24) + df['deterministic'] + df['shipping_time']   # days
    ...
    # cargo_insurance_cost_test.config(text=cargo_insurance_cost.get() * 1000000)
    # salary_voyage_test.config(text=salary_voyage.get())
    # piracy_insurance_cost_test.config(text=piracy_insurance_cost.get())
    # repair_service_vayage_test.config(text=repair_service_vayage.get())
    # voyage_capital_cost_test.config(text=voyage_capital_cost.get())
    # voyage_insurance_cost_test.config(text=voyage_insurance_cost.get())
    # port_dues_test.config(text=port_dues.get())
    # total_environmental_cost_test.config(text=total_environmental_cost.get())
    # deterministic_cost_test.config(text=df['deterministic_cost'].mean())
    # fuel_random_test.config(text=df['fuel_random'].mean())
    # bureaucracy_test.config(text=bureaucracy.get())
    # dwell_test.config(text=dwell.get())
    # deterministic_test.config(text=df['deterministic'].mean())
    # #feul_test.config(text=)
    # fuel_std_test.config(text=df['fuel_random'].std())
    # weather_test.config(text=df['weather'].mean())
    # weather_std_test.config(text=df['weather'].std())
    # speed_test.config(text=df['speed'].mean())
    # speed_std_test.config(text=df['speed'].std())
    # distance_test.config(text=df['distance'].mean())
    # distance_std_test.config(text=df['distance'].std())
    # shipping_time_test.config(text=df['shipping_time'].mean())
    # shipping_time_std_test.config(text=df['shipping_time'].std())
    # bunkering_test.config(text=df['bunkering'].mean())
    # bunkering_std_test.config(text=df['bunkering'].std())
    # fuel_cost_test.config(text=df['fuel_consumption_cost'].mean())
    # fuel_cost_std_test.config(text=df['fuel_consumption_cost'].std())
    # total_time_test.config(text=df['total_time'].mean())
    # total_time_std_test.config(text=df['total_time'].std())
    # total_cost_test.config(text=df['total_cost'].mean())
    # total_cost_std_test.config(text=df['total_cost'].std())
...
def graphTimeCost(df):
    ...
    plot_cost.axvline(df["total_cost"].mean(), color='k', linestyle='dashed', linewidth=1)
    plot_cost.text(23, 45, r'$\mu=15, b=3$')
    plot_cost.legend()
    plot_cost.set_xlabel('Total Cost (Million USD)')
    plot_cost.set_ylabel('Frequency')
    plot_cost.set_title('Total Cost Calculation based on stochastic and deterministic variables',fontsize=8)
    ...
    plot_time.set_title('Total Time Calculation based on stochastic and deterministic variables',fontsize=8)
...
# reference_text = "<h5>REFERENCES</h5>\
# <ul>\
# <li style='font-size:75%;'>Aquaplot. (2022). <i>Connecting Maritime.</i>Aquaplot. Retrieved from <a href='https://www.aquaplot.com/'>www.aquaplot.com</a></li>\
# <li style='font-size:75%;'>Amadeo, K. (2021). <i>Oil Price Forecast 2021-2050. </i>The Balance. Retrieved from \
#     <a href='https://www.thebalance.com/oil-price-forecast-3306219#toc-oil-price-forecast-2025-to-2050.'>www.thebalance.com/oil-price-forecast-3306219#toc-oil-price-forecast-2025-to-2050.</a></li>\
# <li style='font-size:75%;'>Buhaug, O., Corbett, J. J., Endresen, O., Eyring, V., Faber, J., Hanayama, S., Lee, D. S., Lee, D., Lindstad, H., Markowska, A. Z., Mjelde, A., Nelissen, D., Nilsen, J., Pålsson, C., Winebrake, J. J., Wu, W., & Yoshida, K. (2009). (rep.).\
#     <i>Second IMO GHG Study 2009. </i>International Maritime Organization. Retrieved from <a href='https://www.cdn.imo.org/localresources/en/OurWork/Environment/Documents/SecondIMOGHGStudy2009.pdf. '>www.cdn.imo.org/localresources/en/OurWork/Environment/Documents/SecondIMOGHGStudy2009.pdf. </a></li>\
# <li style='font-size:75%;'>Gleb, S., & Jin, J. G. (2021). <i>Evaluating the feasibility of combined use of the Northern Sea Route and the Suez Canal Route considering ice parameters. </i>Transportation Research Part A, 147, 350-369. https://doi.org/10.1016/j.tra.2021.03.024 </li>\
# <li style='font-size:75%;'>Aquaplot. (2022). <i>Connecting Maritime.</i>Aquaplot. Retrieved from <a href='https://www.aquaplot.com/'>www.aquaplot.com</a></li>\
# </ul>\
# "
# reference_label = HTMLScrolledText(tab_references, html=reference_text)
# #description_help_label = HtmlLabel(tab_help, text=description_help, messages_enabled = False)
# #description_help_label.pack(fill="both", ipadx=10, ipady=10, expand=True)
# #description_help_label.grid(column=1, row=1, pady=5, padx=5)
# reference_label.place(x=10, y=10, relwidth=0.98, relheight=0.98)
#################################################################################################################################################################################################
########################################################################################## Developers #################################################################################################
#################################################################################################################################################################################################
# create label frame
#lf_developers = ttk.LabelFrame(tab_developers, text="Developers")
#lf_developers.grid(column=0,row=0, padx=10, pady=7, ipady=5, ipadx=5)

# image_sajjadi_label = HtmlLabel(tab_developers, text="<p><img src='sajjadi.png' alt='Sayed Mohammad Sajjadi' style='width:100px;height:100px;margin-right:25px;'></p>")
# image_sajjadi_label.grid(column=0, row=0, sticky=tk.W)

# description_sajjadi_text = "<p>Sayed Mohammad Sajjadi</p>\
#     <p>Software Developer, Model Creator</p>"
# description_sajjadi_label = HtmlLabel(tab_developers, text=description_sajjadi_text)
# description_sajjadi_label.grid(column=1, row=0, columnspan=2)
################################################################################################ Sajjadi ##########################################################################################
# Frame
...
# Photo
image_giovanni = Image.open("giovanni-modified.png")
...
#################################################################################################################################################################################################
# # create label frame
# lf_references = ttk.LabelFrame(tab_test, text="Time")
# lf_references.grid(column=0,row=0, padx=10, pady=7, ipady=5, ipadx=5)

# # label stochastic fuel price mean
# cargo_insurance_cost_test_label = ttk.Label(lf_references, text="cargo insurance cost: ")
# cargo_insurance_cost_test_label.grid(column=0, row=0, sticky=tk.W)
# cargo_insurance_cost_test = ttk.Label(lf_references)
# cargo_insurance_cost_test.grid(column=1, row=0, sticky=tk.W)

# salary_voyage_test_label = ttk.Label(lf_references, text="salary voyage: ")
# salary_voyage_test_label.grid(column=0, row=1, sticky=tk.W)
# salary_voyage_test = ttk.Label(lf_references)
# salary_voyage_test.grid(column=1, row=1, sticky=tk.W)

# piracy_insurance_cost_label = ttk.Label(lf_references, text="piracy insurance cost: ")
# piracy_insurance_cost_label.grid(column=0, row=2, sticky=tk.W)
# piracy_insurance_cost_test = ttk.Label(lf_references)
# piracy_insurance_cost_test.grid(column=1, row=2, sticky=tk.W)

# repair_service_vayage_label = ttk.Label(lf_references, text="repair service vayage: ")
# repair_service_vayage_label.grid(column=0, row=3, sticky=tk.W)
# repair_service_vayage_test = ttk.Label(lf_references)
# repair_service_vayage_test.grid(column=1, row=3, sticky=tk.W)

# voyage_capital_cost_label = ttk.Label(lf_references, text="voyage capital cost: ")
# voyage_capital_cost_label.grid(column=0, row=4, sticky=tk.W)
# voyage_capital_cost_test = ttk.Label(lf_references)
# voyage_capital_cost_test.grid(column=1, row=4, sticky=tk.W)

# voyage_insurance_cost_label = ttk.Label(lf_references, text="voyage insurance cost: ")
# voyage_insurance_cost_label.grid(column=0, row=5, sticky=tk.W)
# voyage_insurance_cost_test = ttk.Label(lf_references)
# voyage_insurance_cost_test.grid(column=1, row=5, sticky=tk.W)


# port_dues_label = ttk.Label(lf_references, text="port dues: ")
# port_dues_label.grid(column=0, row=6, sticky=tk.W)
# port_dues_test = ttk.Label(lf_references)
# port_dues_test.grid(column=1, row=6, sticky=tk.W)

# total_environmental_cost_label = ttk.Label(lf_references, text="total environmental cost: ")
# total_environmental_cost_label.grid(column=0, row=7, sticky=tk.W)
# total_environmental_cost_test = ttk.Label(lf_references)
# total_environmental_cost_test.grid(column=1, row=7, sticky=tk.W)

# deterministic_cost_label = ttk.Label(lf_references, text="deterministic cost: ")
# deterministic_cost_label.grid(column=0, row=8, sticky=tk.W)
# deterministic_cost_test = ttk.Label(lf_references)
# deterministic_cost_test.grid(column=1, row=8, sticky=tk.W)

# fuel_random_label = ttk.Label(lf_references, text="fuel random: ")
# fuel_random_label.grid(column=0, row=9, sticky=tk.W)
# fuel_random_test = ttk.Label(lf_references)
# fuel_random_test.grid(column=1, row=9, sticky=tk.W)

# bureaucracy_label = ttk.Label(lf_references, text="bureaucracy: ")
# bureaucracy_label.grid(column=0, row=10, sticky=tk.W)
# bureaucracy_test = ttk.Label(lf_references)
# bureaucracy_test.grid(column=1, row=10, sticky=tk.W)

# dwell_label = ttk.Label(lf_references, text="dwell: ")
# dwell_label.grid(column=0, row=11, sticky=tk.W)
# dwell_test = ttk.Label(lf_references)
# dwell_test.grid(column=1, row=11, sticky=tk.W)

# deterministic_label = ttk.Label(lf_references, text="deterministic time: ")
# deterministic_label.grid(column=0, row=12, sticky=tk.W)
# deterministic_test = ttk.Label(lf_references)
# deterministic_test.grid(column=1, row=12, sticky=tk.W)

# fuel_label = ttk.Label(lf_references, text="feul mean: ")
# fuel_label.grid(column=0, row=13, sticky=tk.W)
# feul_test = ttk.Label(lf_references)
# feul_test.grid(column=1, row=13, sticky=tk.W)

# fuel_std_label = ttk.Label(lf_references, text="fuel std: ")
# fuel_std_label.grid(column=0, row=14, sticky=tk.W)
# fuel_std_test = ttk.Label(lf_references)
# fuel_std_test.grid(column=1, row=14, sticky=tk.W)

# weather_label = ttk.Label(lf_references, text="weather mean: ")
# weather_label.grid(column=0, row=15, sticky=tk.W)
# weather_test = ttk.Label(lf_references)
# weather_test.grid(column=1, row=15, sticky=tk.W)

# weather_std_label = ttk.Label(lf_references, text="weather std: ")
# weather_std_label.grid(column=0, row=16, sticky=tk.W)
# weather_std_test = ttk.Label(lf_references)
# weather_std_test.grid(column=1, row=16, sticky=tk.W)

# speed_label = ttk.Label(lf_references, text="speed mean: ")
# speed_label.grid(column=0, row=17, sticky=tk.W)
# speed_test = ttk.Label(lf_references)
# speed_test.grid(column=1, row=17, sticky=tk.W)

# speed_label = ttk.Label(lf_references, text="speed std: ")
# speed_label.grid(column=0, row=18, sticky=tk.W)
# speed_std_test = ttk.Label(lf_references)
# speed_std_test.grid(column=1, row=18, sticky=tk.W)

# distance_label = ttk.Label(lf_references, text="distance mean: ")
# distance_label.grid(column=0, row=19, sticky=tk.W)
# distance_test = ttk.Label(lf_references)
# distance_test.grid(column=1, row=19, sticky=tk.W)

# distance_label = ttk.Label(lf_references, text="distance std: ")
# distance_label.grid(column=0, row=20, sticky=tk.W)
# distance_std_test = ttk.Label(lf_references)
# distance_std_test.grid(column=1, row=20, sticky=tk.W)

# shipping_time_label = ttk.Label(lf_references, text="shipping time mean: ")
# shipping_time_label.grid(column=0, row=21, sticky=tk.W)
# shipping_time_test = ttk.Label(lf_references)
# shipping_time_test.grid(column=1, row=21, sticky=tk.W)

# shipping_time_label = ttk.Label(lf_references, text="shipping time std: ")
# shipping_time_label.grid(column=2, row=21, sticky=tk.W)
# shipping_time_std_test = ttk.Label(lf_references)
# shipping_time_std_test.grid(column=3, row=21, sticky=tk.W)

# bunkering_label = ttk.Label(lf_references, text="bunkering mean: ")
# bunkering_label.grid(column=0, row=22, sticky=tk.W)
# bunkering_test = ttk.Label(lf_references)
# bunkering_test.grid(column=1, row=22, sticky=tk.W)

# bunkering_label = ttk.Label(lf_references, text="bunkering std: ")
# bunkering_label.grid(column=0, row=23, sticky=tk.W)
# bunkering_std_test = ttk.Label(lf_references)
# bunkering_std_test.grid(column=1, row=23, sticky=tk.W)

# fuel_cost_label = ttk.Label(lf_references, text="fuel cost mean: ")
# fuel_cost_label.grid(column=0, row=24, sticky=tk.W)
# fuel_cost_test = ttk.Label(lf_references)
# fuel_cost_test.grid(column=1, row=24, sticky=tk.W)

# fuel_cost_label = ttk.Label(lf_references, text="fuel cost std: ")
# fuel_cost_label.grid(column=0, row=25, sticky=tk.W)
# fuel_cost_std_test = ttk.Label(lf_references)
# fuel_cost_std_test.grid(column=1, row=25, sticky=tk.W)

# total_time_label = ttk.Label(lf_references, text="total time mean: ")
# total_time_label.grid(column=0, row=26, sticky=tk.W)
# total_time_test = ttk.Label(lf_references)
# total_time_test.grid(column=1, row=26, sticky=tk.W)

# total_time_label = ttk.Label(lf_references, text="total time std: ")
# total_time_label.grid(column=0, row=27, sticky=tk.W)
# total_time_std_test = ttk.Label(lf_references)
# total_time_std_test.grid(column=1, row=27, sticky=tk.W)

# total_cost_label = ttk.Label(lf_references, text="total cost mean: ")
# total_cost_label.grid(column=0, row=28, sticky=tk.W)
# total_cost_test = ttk.Label(lf_references)
# total_cost_test.grid(column=1, row=28, sticky=tk.W)

# total_cost_label = ttk.Label(lf_references, text="total cost std: ")
# total_cost_label.grid(column=0, row=29, sticky=tk.W)
# total_cost_std_test = ttk.Label(lf_references)
# total_cost_std_test.grid(column=1, row=29, sticky=tk.W)
